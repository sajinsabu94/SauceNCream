...
namespace WindowsFormsApplication2
{
    public partial class Form3 : Form
    {
        ...
        private void Load1()
        {
            ...
            if (count > 0)
            {
                try
                {
                    ...
                    cmd.CommandText = "select tb1.Item,tb1.Qnty,tb2.Rate,tb2.Vat,(tb1.Qnty*tb2.Rate) as Amount from (Select Item,sum(Qnty) as Qnty from Bill where Dat = '" + date + "' Group By Item) AS tb1 inner join (Select PdNam,Rate,Vat from StockDetls UNION Select Item,Rate,Vat from Others) AS tb2 on tb1.Item = tb2.PdNam";
                    conn.Open();
                    //MessageBox.Show(count.ToString());
                    int i, j;
        ...
        private void Load2()
        {
            ...
            //MessageBox.Show(date);            //Now.ToString("dd-MM-yyyy");
            string stmt = "SELECT COUNT(Distinct Item) FROM Bill where Dat like '___"+date+"_"+yr+"'";
            int count = 0;
            using (SqlConnection thisConnection = new SqlConnection("Data Source=.\\SQLEXPRESS;AttachDbFilename=|DataDirectory|\\App_Data\\SauceCream.mdf;Integrated Security=True;User Instance=True"))
            {
                using (SqlCommand cmdCount = new SqlCommand(stmt, thisConnection))
           ...
           if (count > 0)
            ...
            cmd.CommandText = "select tb1.Item,tb1.Qnty,tb2.Rate,tb2.Vat,(tb1.Qnty*tb2.Rate) as Amount from (Select Item,sum(Qnty) as Qnty from Bill where Dat like '___" + date + "%' Group By Item) AS tb1 inner join (Select PdNam,Rate,Vat from StockDetls UNION Select Item,Rate,Vat from Others) AS tb2 on tb1.Item = tb2.PdNam";
        ...
        private void Load3()
        {
            ...
            cmd.CommandText = "select tb1.Item,tb1.Qnty,tb2.Rate,tb2.Vat,(tb1.Qnty*tb2.Rate) as Amount from (Select Item,sum(Qnty) as Qnty from Bill where Dat like '______" + date + "%' Group By Item) AS tb1 inner join (Select PdNam,Rate,Vat from StockDetls UNION Select Item,Rate,Vat from Others) AS tb2 on tb1.Item = tb2.PdNam";
            conn.Open();
            //MessageBox.Show(count.ToString());
            int i, j;
            //int row2 = 0;
